...
# --- ProjectMember Schemas (修正！) ---
class ProjectMemberBase(BaseModel):
    display_name: str
    # ユーザーの要望を反映: departmentとroleは任意とし、デフォルト値も設定
    department: Optional[str] = "Unassigned"
    role: str = "Member"
class ProjectMemberCreate(ProjectMemberBase):
    project_id: int
    account_id: Optional[int] = None # account_idは任意
...
@app.get("/", tags=["Root"])
def read_root(): return {"message": "MOTK Backend is running with the new schema!"}
...
def create_project_member(member: ProjectMemberCreate, db: Session = Depends(get_db)):
    ...
    # ユーザーの要望を反映: account_idが 0 または None の場合、DBにはNULLとして保存
    if member.account_id is not None:
        if member.account_id == 0:
            member.account_id = None # 0をNoneに変換
...
# --- Project, Shot, Asset, Task Endpoints (変更なし) ---
